...
import styles from './TimetableWorkersSidebar.module.scss';

interface TimetableWorkersSidebarProps {
  className?: string;
}

const TimetableWorkersSidebar: FC<TimetableWorkersSidebarProps> = observer(({ className }) => {
        ...
        <div key={job}>
          ...
          {Object.entries(workersByRole).map(([workerId, workerData]) => (
            <div key={`${job}-${workerId}`} className={styles['worker-cell']}>
              <div className={styles['worker-pic__container']} />
